...
void MoveGauge::ChangeColor(unsigned int _top, unsigned int _bottom, unsigned int _frame, unsigned int _back) {
	topColor = _top;
	bottomColor = _bottom;
	frameColor = _frame;
	backColor = _back;
...
void MoveGauge::Update() {
	if (!isActive ||  !currentValue || !maxValue) return;

	MoveUIBase::Update();
	VECTOR pos = MyImage::BoxCenterPos(x, y, w, h);
